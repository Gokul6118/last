...
const api = {
  getTodos: async (): Promise<Todo[]> => {
    const res = await fetch(`https://last-hono-backend-z7ra.vercel.app/`);
    if (!res.ok) throw new Error("Failed to fetch todos");
    return res.json();
  },

  addTodo: async (data: TodoForm) => {
    const res = await fetch(`https://last-hono-backend-z7ra.vercel.app/todos`, {
      method: "POST",
  ...
  updateTodo: async (id: number, data: Todo) => {
    const res = await fetch(`https://last-hono-backend-z7ra.vercel.app/todos/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
